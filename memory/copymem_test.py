...
def train(ep):
    ...
    for batch_idx, batch in enumerate(range(0, n_train, batch_size)):
        ...
        optimizer.zero_grad()
        # print(x.unsqueeze(2).contiguous().shape)
        print(x.unsqueeze(2).contiguous())
        print(x.unsqueeze(2).contiguous().shape)
        sys.exit()
        out = model(x.unsqueeze(2).contiguous())
        # print(out.shape)

        # print(out.view(-1, n_classes).shape, y.view(-1).shape)
        loss = criterion(out.view(-1, n_classes), y.view(-1))
        pred = out.view(-1, n_classes).data.max(1, keepdim=True)[1]
        # print(pred)
        out.view(-1, n_classes)
        # sys.exit()
        ...
        if batch_idx > 0 and batch_idx % args.log_interval == 0:
            ...
            print('| Epoch {:3d} | {:5d}/{:5d} batches | lr {:2.5f} | ms/batch {:5.2f} | '
                ...
                avg_loss.item(), 100. * correct / counter))
            start_time = time.time()
            total_loss = 0
            correct = 0
            counter = 0
